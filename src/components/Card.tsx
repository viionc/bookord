...
const Card: React.FC<Message> = (message: Message) => {
    const {currentUserProfile, userDatabase, deleteMessage} = useFirebaseContext();
    ...
    return (
        <Container className="mt-2 d-flex pt flex-column text-white">
            <div className="d-flex jutify-content-center align-items-center gap-2">
                ...
                {message.userUid === currentUserProfile?.uid ||
                currentUserProfile?.roles.includes("moderator") ? (
                    <span className="trash-icon" onClick={handleDeleteMessage}>
                        <svg
